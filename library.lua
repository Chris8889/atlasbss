...
for i, v in pairs(game:GetService("CoreGui"):GetDescendants()) do
	if v:IsA("TextLabel") then
		if v.Text:find("dagydgqwduwqgurqw") then
...
do -- imgui
	...
	Indicator_2.Position = UDim2.new(0.899999976, 0, 0.100000001, 0)
...
-- script
local imgui = game:GetService("CoreGui"):FindFirstChild(string)
...
local function visible(Window, options)
	library.Enabled = not library.Enabled

	if library.Enabled then
		TweenService:Create(Window, TweenInfo.new(0.5, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), {
			Size = UDim2.new(0, options.min_size.X, 0, options.min_size.Y)
		}):Play()
	else
		local main_tween = TweenService:Create(Window, TweenInfo.new(0.5, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), {
			Size = UDim2.new(0, options.min_size.X, 0, 27)
		}):Play()
	end
end

local function visible2(Window, options)
	library.Enabled = not library.Enabled
	...
	if library.Enabled then
		...
		local main_tween = TweenService:Create(Window, TweenInfo.new(0.5, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), {
			Size = UDim2.new(0, options.min_size.X, 0, 0)
		})
...
function library:AddWindow(title, options)
	...
	Window.ZIndex = Window.ZIndex + (library.windows * 10)
	local window_data = {}
	...
	UIS.InputBegan:Connect(function(input, gameProcessed)
		if input.KeyCode == options.toggle_key then
			if imgui then
				visible2(Window, options)
			end
		end
	end)
	...
	UIS.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
			...
			TweenService:Create(Window, TweenInfo.new(0), {
				Position = position
			}):Play()
        end
    end)

	Window:FindFirstChild("Bar").InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            library.dragging = true
            library.drag_position = input.Position
            library.start_position = Window.Position
	...
	Window:FindFirstChild("Bar"):FindFirstChild("Toggle").MouseButton1Click:Connect(function()
		visible(Window, options)
	end)
	...
	do -- UI Elements
		...
		do -- Add Tab
			function window_data:AddTab(tab_name)
				...
				new_button.ZIndex = new_button.ZIndex + (library.windows * 10)
				new_button:GetChildren()[1].ZIndex = new_button:GetChildren()[1].ZIndex + (library.windows * 10)

				local new_tab = Prefabs:FindFirstChild("Tab"):Clone()
				new_tab.Parent = tabs
				new_tab.ZIndex = new_tab.ZIndex + (library.windows * 10)
				...
				do -- Tab Elements
					function tab_data:AddLabel(label_text) -- [Label]
						...
						label.ZIndex = label.ZIndex + (library.windows * 10)
						return label
					end
					...
					function tab_data:AddButton(button_text, callback) -- [Button]
						...
						button.ZIndex = button.ZIndex + (library.windows * 10)
						button:GetChildren()[1].ZIndex = button:GetChildren()[1].ZIndex + (library.windows * 10)
						...
						button.MouseButton1Click:Connect(function()
							spawn(function()
								...
								local new_x = mouse.X - circle.AbsolutePosition.X
								local new_y = mouse.Y - circle.AbsolutePosition.Y
					...
					function tab_data:AddSwitch(switch_text, callback) -- [Switch]
						...
						switch:FindFirstChild("Title").ZIndex = switch:FindFirstChild("Title").ZIndex + (library.windows * 10)
						switch.ZIndex = switch.ZIndex + (library.windows * 10)
						switch:GetChildren()[1].ZIndex = switch:GetChildren()[1].ZIndex + (library.windows * 10)
					...
					function tab_data:AddTextBox(textbox_text, callback, textbox_options)
						...
						textbox.ZIndex = textbox.ZIndex + (library.windows * 10)
						textbox:GetChildren()[1].ZIndex = textbox:GetChildren()[1].ZIndex + (library.windows * 10)
					...
					function tab_data:AddSlider(slider_text, callback, slider_options)
						...
						slider.ZIndex = slider.ZIndex + (library.windows * 10)

						local title = slider:FindFirstChild("Title")
						local indicator = slider:FindFirstChild("Indicator")
						local value = slider:FindFirstChild("Value")
						title.ZIndex = title.ZIndex + (library.windows * 10)
						indicator.ZIndex = indicator.ZIndex + (library.windows * 10)
						value.ZIndex = value.ZIndex + (library.windows * 10)
						...
						do -- Slider Math
									...
									spawn(function() -- Loop check
										if Entered and not slider_options.readonly then
											while Held and (not dropdown_open) do
												...
												local success, err = pcall(callback, sel_value)
												if not success then
													error("Caught an error: " .. err)
												end
							...
							function slider_data:Set(new_value)
								...
								new_value = (((new_value >= slider_options.min or 0 and new_value <= slider_options.max or 100) and new_value) / slider_options.max or 100)
					...
					function tab_data:AddDropdown(dropdown_name, callback, wide)
						wide = wide or false
						local dropdown_data = {}
						...
						dropdown.ZIndex = dropdown.ZIndex + (library.windows * 10)
						box.ZIndex = box.ZIndex + (library.windows * 10)
						objects.ZIndex = objects.ZIndex + (library.windows * 10)
						indicator.ZIndex = indicator.ZIndex + (library.windows * 10)
						dropdown:GetChildren()[3].ZIndex = dropdown:GetChildren()[3].ZIndex + (library.windows * 10)
						if wide then dropdown.Size = UDim2.new(0, 250, 0, 20) end
						...
						function dropdown_data:Add(n)
							...
							object.Text = "   " .. n
							object.ZIndex = object.ZIndex + (library.windows * 10)
							...
							object.MouseButton1Click:Connect(function()
								if dropdown_open then
									...
									local success, err = pcall(callback, n)
									if not success then
										error("Caught an error: " .. err)
									end
								end
					...
					function tab_data:AddColorPicker(callback)
						...
						color_picker.ZIndex = color_picker.ZIndex + (library.windows * 10)
						...
						palette.ZIndex = palette.ZIndex + (library.windows * 10)
						sample.ZIndex = sample.ZIndex + (library.windows * 10)
						saturation.ZIndex = saturation.ZIndex + (library.windows * 10)
						...
						do -- Color Picker Math
							...
							palette_indicator.ZIndex = palette_indicator.ZIndex + (library.windows * 10)
							saturation_indicator.ZIndex = saturation_indicator.ZIndex + (library.windows * 10)
					...
					function tab_data:AddFolder(folder_name) -- [Folder]
						...
						folder.ZIndex = folder.ZIndex + (library.windows * 10)
						button.ZIndex = button.ZIndex + (library.windows * 10)
						objects.ZIndex = objects.ZIndex + (library.windows * 10)
						toggle.ZIndex = toggle.ZIndex + (library.windows * 10)
						button:GetChildren()[1].ZIndex = button:GetChildren()[1].ZIndex + (library.windows * 10)
